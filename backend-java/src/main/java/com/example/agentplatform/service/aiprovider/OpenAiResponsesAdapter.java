...
@Component
public class OpenAiResponsesAdapter extends ProviderAdapterSupport implements AiProviderProtocolAdapter {
    private final RestTemplate restTemplate;

    public OpenAiResponsesAdapter(RestTemplate restTemplate) {
        this.restTemplate = restTemplate;
    }

    @Override
    public boolean supports(String apiFormat) {
        return "openai_responses".equals(apiFormat);
    }
    ...
    public ProviderTestResult test(ProviderTestRequest request) {
        try {
            ...
            restTemplate.postForEntity(endpoint(request.getBaseUrl(), "/v1", "/responses"),
