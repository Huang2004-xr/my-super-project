    ...
    def list_capabilities(self) -> List[CapabilityDefinition]:
        return [
            CapabilityDefinition(
                key="TEXT_CHAT",
                name="文字沟通",
                description="使用本地 Ollama 或已配置的外部模型进行自由聊天、问题解释和普通文本生成。",
                examplePrompt="帮我总结一下超级 Agent 能做什么",
            ),
            CapabilityDefinition(
                key="VIDEO_CREATION",
                name="视频创作",
                description="生成短视频脚本、分镜、口播和执行建议。",
                examplePrompt="帮我生成一条介绍产品卖点的短视频脚本",
            ),
            CapabilityDefinition(
                key="IMAGE_CREATION",
                name="图片创作",
                description="生成图片提示词、风格、构图和负面提示词。",
                examplePrompt="帮我生成一张科技感产品海报",
            ),
            CapabilityDefinition(
                key="KNOWLEDGE_RETRIEVAL",
                name="知识库检索",
                description="先检索内置知识库，再调用模型生成问答结果。",
                examplePrompt="根据知识库说明，超级 Agent 支持哪些能力？",
            ),
        ]
    ...
    def start_run(self, request: CreateAgentRunRequest) -> AgentRun:
        ...
        try:
            ...
            if provider_config:
                ...
                trace.add("llm.request_id", str(provider_config.get("requestId") or ""))
            else:
    ...
    def _workflow(self, capability: str) -> tuple[List[AgentStep], str, str, str]:
        if capability == "VIDEO_CREATION":
            return (
                [
                    ...
                    self._step("llm", "生成视频文本方案", "调用模型生成脚本、分镜和建议"),
                ],
                "media.plan_video",
                "video_creation",
                "视频创作结果",
        ...
        if capability == "IMAGE_CREATION":
            return (
                [
                    ...
                    self._step("llm", "生成图片提示词", "调用模型生成提示词和设计说明"),
                ],
        ...
        if capability == "KNOWLEDGE_RETRIEVAL":
            return (
                [
                    ...
                    self._step("llm", "生成知识库回答", "调用模型基于片段生成回答"),
                ],
        ...
        return (
            [
                ...
                self._step("llm", "生成文字回复", "调用模型生成自然语言回复"),
            ],
            "chat.respond",
            "text_chat",
            "文字沟通结果",
